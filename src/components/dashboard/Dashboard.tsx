...
// Tailwind v4 emits oklch() directly in utility class rules (not via var()),
// so overriding CSS variables alone doesn't help. Instead we rewrite every
// <style> element in the cloned document, converting oklch() values to hex
// using the standard oklch → oklab → linear-sRGB → sRGB conversion.
...
function oklchToHex(L: number, C: number, H: number): string {
  ...
  // linear → gamma-encoded sRGB, clamp, to 8-bit
  const toU8 = (c: number): number => {
    const v = Math.max(0, Math.min(1, c));
    const g = v <= 0.0031308 ? 12.92 * v : 1.055 * Math.pow(v, 1 / 2.4) - 0.055;
    return Math.round(g * 255);
  };
  const r = toU8(lr).toString(16).padStart(2, "0");
  const g = toU8(lg).toString(16).padStart(2, "0");
  const bh = toU8(lb).toString(16).padStart(2, "0");
  return `#${r}${g}${bh}`;
}
